...
export default function DevelopmentSection({ canAccess }) {
  ...
  const activeDevTab = searchParams.get("devTab") === "models" ? "models" : "endpoints";
  const endpointsQuery = useAdminEndpointsQuery(canAccess && activeDevTab === "endpoints");
  const modelsQuery = useAdminModelsQuery(canAccess && activeDevTab === "models");

  const handleDevTabChange = (tabKey) => {
    const next = new URLSearchParams(searchParams);
    next.set("devTab", tabKey);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2">
        <SecondaryButton
          className={`px-3 py-1.5 text-sm ${activeDevTab === "endpoints" ? "border-gray-900 text-gray-900" : ""}`}
          onClick={() => handleDevTabChange("endpoints")}
        >
          Endpoints
        </SecondaryButton>
        <SecondaryButton
          className={`px-3 py-1.5 text-sm ${activeDevTab === "models" ? "border-gray-900 text-gray-900" : ""}`}
          onClick={() => handleDevTabChange("models")}
        >
          Modelos
        </SecondaryButton>
      </div>

      {activeDevTab === "endpoints" ? (
