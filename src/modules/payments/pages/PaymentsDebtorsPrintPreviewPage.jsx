...
export default function PaymentsDebtorsPrintPreviewPage() {
          ...
          <section className="mx-auto w-[210mm] max-w-full bg-white p-[12mm] shadow-sm print:shadow-none">
            <header className="border-b border-gray-300 pb-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h1 className="text-xl font-semibold text-gray-900">Lista de deudores</h1>
                  <div className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-600">
                    <span>Generado el {formatDate(payload?.generatedAt)}</span>
                    <span>Total seleccionados: {items.length}</span>
                  </div>
                </div>
              </div>
            </header>
            ...
            <div className="mt-5 overflow-hidden rounded-lg border border-gray-200">
              <table className="min-w-full text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-3 py-2 text-left font-semibold text-gray-700">Alumno</th>
                    <th className="px-3 py-2 text-left font-semibold text-gray-700">DNI</th>
                    <th className="px-3 py-2 text-left font-semibold text-gray-700">Codigo</th>
                    <th className="px-3 py-2 text-left font-semibold text-gray-700">Salon</th>
                    <th className="px-3 py-2 text-left font-semibold text-gray-700">Pendiente</th>
                    <th className="px-3 py-2 text-left font-semibold text-gray-700">Vencido</th>
                  </tr>
                </thead>
                <tbody>
                  {items.map((item) => (
                    <tr key={item.studentId} className="border-t border-gray-100">
                      <td className="px-3 py-2 text-gray-900">{item.fullName || "-"}</td>
                      <td className="px-3 py-2 text-gray-700">{item.dni || "-"}</td>
                      <td className="px-3 py-2 text-gray-700">{item.code || "-"}</td>
                      <td className="px-3 py-2 text-gray-700">{item.classroomLabel || "-"}</td>
                      <td className="px-3 py-2 font-medium text-gray-900">{formatMoney(item.totalPending)}</td>
                      <td className="px-3 py-2 font-medium text-amber-700">{formatMoney(item.totalOverdue)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
